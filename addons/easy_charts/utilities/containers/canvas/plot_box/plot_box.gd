extends Control
class_name FW_PlotBox

signal function_point_entered(point, function)
signal function_point_exited(point, function)
@onready var tooltip: FW_DataTooltip = $Tooltip

var focused_point: FW_Point
var focused_function: FW_Function
...
func _on_point_entered(point: FW_Point, function: FW_Function, props: Dictionary = {}) -> void:
    ...
    var x_value: String = x_labels_function.call(point.value.x) if not x_labels_function.is_null() else \
        point.value.x if point.value.x is String else FW_ECUtilities._format_value(point.value.x, FW_ECUtilities._is_decimal(point.value.x))
    var y_value: String = y_labels_function.call(point.value.y) if not y_labels_function.is_null() else \
        point.value.y if point.value.y is String else FW_ECUtilities._format_value(point.value.y, FW_ECUtilities._is_decimal(point.value.y))
    var color: Color = function.get_color() if function.get_type() != FW_Function.Type.PIE \
...
func _on_point_exited(point: FW_Point, function: FW_Function) -> void:
