...
func _draw_origin() -> void:
    var xorigin: float = FW_ECUtilities._map_domain(0.0, x_domain, { lb = self.plot_box.position.x, ub = self.plot_box.end.x })
    var yorigin: float = FW_ECUtilities._map_domain(0.0, y_domain, { lb = self.plot_box.end.y, ub = self.plot_box.position.y })
...
func _draw_vertical_grid() -> void:
    ...
    for _x in (scaler + 1):
        ...
        var x_val: float = FW_ECUtilities._map_domain(x_sampled_val, { lb = self.plot_box.position.x, ub = self.plot_box.end.x }, x_domain)
...
func _draw_horizontal_grid() -> void:
    ...
    for _y in (scaler + 1):
        ...
        var y_val: float = FW_ECUtilities._map_domain(y_sampled_val, { lb = self.plot_box.end.y, ub = self.plot_box.position.y }, y_domain)
...
func _get_tick_label(line_index: int, line_value: float, axis_has_decimals: bool, labels_function: Callable) -> String:
    ...
    if labels_function.is_null():
        tick_lbl = FW_ECUtilities._format_value(line_value, axis_has_decimals)
    else:
