@tool
extends RefCounted
class_name FW_MatrixGenerator

static func zeros(rows: int, columns: int) -> FW_Matrix:
	var zeros: Array = []
	var t_rows: Array = []
	...
	return FW_Matrix.new(zeros)

# Generates a Matrix with random values between [from; to] with a given @size (rows, columns)
static func random_float_range(from : float, to : float, size : Vector2, _seed : int = 1234) -> FW_Matrix:
	seed(_seed)
	randomize()
	var array : Array = []
	...
	return FW_Matrix.new(array)

# Generates a Matrix giving an Array (Array must by Array[Array])
static func from_array(array : Array = []) -> FW_Matrix:
	var matrix : Array = []
	matrix.append(array)
	return FW_Matrix.new(matrix)

# Generates a sub-Matrix giving a Matrix, a @from Array [row_i, column_i] and a @to Array [row_j, column_j]
static func sub_matrix(_matrix : FW_Matrix, from : PackedInt32Array, to : PackedInt32Array) -> FW_Matrix:
...
static func duplicate(_matrix : FW_Matrix) -> FW_Matrix:
	return FW_Matrix.new(_matrix.to_array().duplicate())

# Calculate the determinant of a matrix
static func determinant(matrix: FW_Matrix) -> float:
	...
	if matrix.rows() == 2 :
		...
		var submatrix: FW_Matrix = sub_matrix(matrix, [1, 0], [matrix.rows(), matrix.columns()])
		for j in matrix.columns() :
			var cofactor: FW_Matrix = copy(submatrix)
...
static func inverse(matrix: FW_Matrix) -> FW_Matrix:
	var inverse: FW_Matrix
	
	# Minors and Cofactors
	var minors_cofactors: FW_Matrix = zeros(matrix.rows(), matrix.columns())
	var multiplier: float = -1.0
	
	for i in minors_cofactors.rows():
		for j in minors_cofactors.columns():
			var t_minor: FW_Matrix = copy(matrix)
			t_minor.remove_row(i)
			t_minor.remove_column(j)
			multiplier *= -1.0
			minors_cofactors.set_value(multiplier * determinant(t_minor), i, j)
	
	var transpose: FW_Matrix = transpose(minors_cofactors)
...
static func transpose(_matrix : FW_Matrix) -> FW_Matrix:
	var array : Array = []
...
static func dot(_matrix1 : FW_Matrix, _matrix2 : FW_Matrix) -> FW_Matrix:
...
static func hadamard(_matrix1 : FW_Matrix, _matrix2 : FW_Matrix) -> FW_Matrix:
...
static func multiply_int(_matrix1 : FW_Matrix, _int : int) -> FW_Matrix:
...
static func multiply_float(_matrix1 : FW_Matrix, _float : float) -> FW_Matrix:
	var array : Array = _matrix1.to_array().duplicate()
	for x in range(_matrix1.to_array().size()):
		for y in range(_matrix1.to_array()[x].size()):
			array[x][y]*=_float
...
static func copy(matrix: FW_Matrix) -> FW_Matrix:
	return FW_Matrix.new(matrix.values.duplicate(true))
