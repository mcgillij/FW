@tool
extends Resource
class_name FW_DataFrame

var table_name : String = ""
var labels : PackedStringArray = []
var headers : PackedStringArray = []
var datamatrix : FW_Matrix = null
var dataset : Array = []

func _init(datamatrix : FW_Matrix, headers : PackedStringArray = [], labels : PackedStringArray = [] , table_name : String = "") -> void:
	...
	if headers.is_empty() : for header in range(datamatrix.get_size().y) : headers.append(FW_MatrixGenerator.get_letter_index(header))
	build_dataframe(datamatrix, headers, labels, table_name)

func build_dataframe(datamatrix : FW_Matrix, headers : PackedStringArray = [], labels : PackedStringArray = [] , table_name : String = "") -> void:
...
func build_dataset_from_matrix(datamatrix : FW_Matrix, headers : PackedStringArray, labels : PackedStringArray) -> Array:
...
func insert_column(column : Array, header : String = "", index : int = dataset[0].size() - 1) -> void:
	...
	headers.insert(index, header if header != "" else FW_MatrixGenerator.get_letter_index(index))
...
func transpose():
	build_dataframe(FW_MatrixGenerator.transpose(datamatrix), labels, headers, table_name)
...
func columns(headers : PackedStringArray) -> FW_Matrix:
	var values : Array = []
	for header in headers:
		values.append(get_column(header))
	return FW_MatrixGenerator.transpose(FW_Matrix.new(values))
...
func rows(labels : Array) -> FW_Matrix:
	var values : Array = []
	for label in labels:
		values.append(get_row(label))
	return FW_Matrix.new(values)
