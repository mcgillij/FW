...
static func _map_domain(value: float, from_domain: Dictionary, to_domain: Dictionary) -> float:
	if not from_domain.has("lb") or not from_domain.has("ub"):
		printerr("EasyCharts: missing domain bounds in", from_domain)
		return value
	if not to_domain.has("lb") or not to_domain.has("ub"):
		printerr("EasyCharts: missing target bounds in", to_domain)
		return value
