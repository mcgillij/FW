...
var focused_bar_midpoint: FW_Point
...
func sample(x_sampled_domain: Dictionary, y_sampled_domain: Dictionary) -> void:
	...
	for i in function.__x.size():
		var top: Vector2 = Vector2(
			FW_ECUtilities._map_domain(i, x_domain, x_sampled_domain),
			FW_ECUtilities._map_domain(function.__y[i], y_domain, y_sampled_domain)
		)
		var base: Vector2 = Vector2(top.x, FW_ECUtilities._map_domain(0.0, y_domain, y_sampled_domain))
		bars.push_back(top)
		bars.push_back(base)
	...
	if event is InputEventMouse:
		for i in bars_rects.size():
			if bars_rects[i].grow(5).abs().has_point(get_relative_position(event.position)):
				var point: FW_Point = FW_Point.new(bars_rects[i].get_center(), { x = function.__x[i], y = function.__y[i]})
