extends FW_FunctionPlotter
class_name FW_ScatterPlotter

signal point_entered(point, function)
signal point_exited(point, function)

var points: Array[FW_Point]
...
func _draw() -> void:
    ...
    if function.get_marker() != FW_Function.Marker.NONE:
        for point in points:
...
func sample(x_sampled_domain: Dictionary, y_sampled_domain: Dictionary) -> void:
    ...
    for i in range(lower_bound, function.__x.size()):
        var _position: Vector2 = Vector2(
            FW_ECUtilities._map_domain(float(function.__x[i]), x_domain, x_sampled_domain),
            FW_ECUtilities._map_domain(float(function.__y[i]), y_domain, y_sampled_domain)
        )
        points.push_back(FW_Point.new(_position, { x = function.__x[i], y = function.__y[i] }))
        points_positions.push_back(_position)

func draw_function_point(point_position: Vector2) -> void:
    match function.get_marker():
        FW_Function.Marker.SQUARE:
            draw_rect(
                Rect2(point_position - (Vector2.ONE * point_size), (Vector2.ONE * point_size * 2)), 
                function.get_color(), true, 1.0
            )
        FW_Function.Marker.TRIANGLE:
