...
@onready var _canvas: FW_Canvas = $Canvas
@onready var plot_box: FW_PlotBox = $"%PlotBox"
@onready var grid_box: FW_GridBox = $"%GridBox"
@onready var functions_box: Control = $"%FunctionsBox"
@onready var function_legend: FW_FunctionLegend = $"%FunctionLegend"
...
var chart_properties: FW_ChartProperties = null
...
func plot(functions: Array[FW_Function], properties: FW_ChartProperties = FW_ChartProperties.new()) -> void:
...
func get_function_plotter(function: FW_Function) -> FW_FunctionPlotter:
    var plotter: FW_FunctionPlotter
    match function.get_type():
        FW_Function.Type.LINE:
            plotter = FW_LinePlotter.new(function)
        FW_Function.Type.AREA:
            plotter = FW_AreaPlotter.new(function)
        FW_Function.Type.PIE:
            plotter = FW_PiePlotter.new(function)
        FW_Function.Type.BAR:
            plotter = FW_BarPlotter.new(function)
        FW_Function.Type.SCATTER, _:
            plotter = FW_ScatterPlotter.new(function)
    return plotter

func load_functions(functions: Array[FW_Function]) -> void:
    self.x = []
    self.y = []
    ...
    for function in functions:
        ...
        var function_plotter: FW_FunctionPlotter = get_function_plotter(function)
        ...
        match function.get_type():
            FW_Function.Type.PIE:
                for i in function.__x.size():
                    ...
                    function_legend.add_label(function.get_type(), interp_color, FW_Function.Marker.NONE, function.__y[i])
            _:
...
func _draw() -> void:
    ...
    if x_domain.is_empty() or y_domain.is_empty():
        printerr("Chart: domain calculation failed", { "x": x, "y": y, "x_domain": x_domain, "y_domain": y_domain })
        return
    ...
    for function_plotter in functions_box.get_children():
        if function_plotter is FW_FunctionPlotter:
...
func calculate_domain(values: Array) -> Dictionary:
    for value_array in values:
        if FW_ECUtilities._contains_string(value_array):
            return { lb = 0.0, ub = (value_array.size() - 1), has_decimals = false , fixed = false }
    var min_max: Dictionary = FW_ECUtilities._find_min_max(values)
    
    if not chart_properties.smooth_domain:
        return { lb = min_max.min, ub = min_max.max, has_decimals = FW_ECUtilities._has_decimals(values), fixed = false }
    else:
        return { lb = FW_ECUtilities._round_min(min_max.min), ub = FW_ECUtilities._round_max(min_max.max), has_decimals = FW_ECUtilities._has_decimals(values) , fixed = false }

func set_x_domain(lb: Variant, ub: Variant) -> void:
    x_domain = { lb = lb, ub = ub, has_decimals = FW_ECUtilities._has_decimals([lb, ub]), fixed = true }

func set_y_domain(lb: Variant, ub: Variant) -> void:
    y_domain = { lb = lb, ub = ub, has_decimals = FW_ECUtilities._has_decimals([lb, ub]), fixed = true }
...
func calculate_plotbox_margins(x_domain: Dictionary, y_domain: Dictionary) -> Vector2:
    ...
    if chart_properties.show_tick_labels:
        ...
        var y_max_formatted: String = FW_ECUtilities._format_value(y_domain.ub, y_domain.has_decimals)
        if y_domain.lb < 0: # negative number
            var y_min_formatted: String = FW_ECUtilities._format_value(y_domain.lb, y_domain.has_decimals)
