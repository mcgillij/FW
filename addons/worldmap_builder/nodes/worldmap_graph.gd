@tool
class_name FW_WorldmapGraph
extends FW_WorldmapViewItem
...
enum ConnectionMode {
	...
	CUSTOM, ## All [member connection_costs] can be changed. X and Y determine the cost of moving FROM the corresponding node. will be multiplied by the nodes' [member FW_WorldmapNodeData.cost].
}
...
var node_datas : Array[FW_WorldmapNodeData]
...
func _ready():
	z_index = -1  # Render editor preview lines behind nodes
...
func add_node(pos : Vector2, parent_node : int, node_data : FW_WorldmapNodeData = null) -> int:
	if node_data == null:
...
func change_node(index: int, node_data: FW_WorldmapNodeData = null) -> int:
...
func get_node_data(index : int) -> FW_WorldmapNodeData:
...
func _draw():
	...
	for x in _node_controls:
		x.hide()
	var offset := Vector2(700, 0) # offset used for editor mode
	if is_editor:
		for x in connection_nodes:
			#FW_Debug.debug_log([node_positions[x.x] , node_positions[x.y]])
			draw_line(node_positions[x.x] - offset , node_positions[x.y] - offset, Color.ORANGE_RED, 4.0)
	...
	for i in node_datas.size():
		...
		node.show()
		if is_editor:
			draw_texture(tex, node_positions[i] - offset - tex_size * 0.5)
