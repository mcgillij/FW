class_name FW_WorldmapViewItem
extends Node2D

## Emitted when a node on this view item receives input.
signal node_gui_input(event : InputEvent, uid : int, resource : FW_WorldmapNodeData)
## Emitted when a node on this view item has the mouse over.
signal node_mouse_entered(uid : int, resource : FW_WorldmapNodeData)
## Emitted when a node on this view item no longer has the mouse over.
signal node_mouse_exited(uid : int, resource : FW_WorldmapNodeData)
...
## Impossible connections should return [code]INF[/code], others return the second node's data's [FW_WorldmapNodeData.cost].
