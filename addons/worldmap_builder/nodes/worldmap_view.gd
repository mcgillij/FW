@tool
class_name FW_WorldmapView
extends Control

class ConnectionPoint extends RefCounted:
	var position := Vector2.ZERO
	var id := 0
	var items : Array[FW_WorldmapViewItem] = []:
		set(v): return
	...
	func add(item : FW_WorldmapViewItem, path : NodePath, node_index : int):
		items.append(item)
...
signal node_gui_input(event : InputEvent, path : NodePath, node_in_path : int, resource : FW_WorldmapNodeData)
## Emitted when a node on this map gets the mouse over it.
signal node_mouse_entered(path : NodePath, node_in_path : int, resource : FW_WorldmapNodeData)
## Emitted when a node on this map no longer has the mouse over.
signal node_mouse_exited(path : NodePath, node_in_path : int, resource : FW_WorldmapNodeData)

## Nodes are only able to be activated if this cost exceeds or equals the node's [member FW_WorldmapNodeData.cost]. [br]
...
@export var initial_item : FW_WorldmapViewItem
...
func _draw():
	...
	for i in children.size():
		...
		var cur_datas : Array[FW_WorldmapNodeData] = []
		var cur_styles : Array[FW_WorldmapStyle] = []
	...
	for i in node_positions.size():
		...
		var cur_datas : Array[FW_WorldmapNodeData] = node_datas[i]
		var cur_styles : Array[FW_WorldmapStyle] = node_styles[i]
...
func get_node_minimum_rect() -> Rect2:
	...
	for x in get_children():
		...
		for j in x.get_node_count():
			...
			var node_data : FW_WorldmapNodeData = x.get_node_data(j)
...
func get_node_data(item : NodePath, node : int) -> FW_WorldmapNodeData:
...
func get_world_state() -> Array:
	return _worldmap_state.values()[0].duplicate(true)
...
func view_item_node_added(node : FW_WorldmapViewItem, index : int):
...
func view_item_node_removed(node : FW_WorldmapViewItem, index : int):
	var node_path := get_path_to(node)
...
func _update_activatable_local(item_path : NodePath):
	var item : FW_WorldmapViewItem = get_node(item_path)
...
func _update_activatable_interitem(connection : ConnectionPoint):
	...
	var filled_node : FW_WorldmapViewItem = get_node(filled_node_path)
...
func _on_child_entered_tree(child : Node):
	if !child is FW_WorldmapViewItem:
		return
...
func _on_node_gui_input(event : InputEvent, node_id : int, resource : FW_WorldmapNodeData, path : NodePath):
	node_gui_input.emit(event, path, node_id, resource)


func _on_node_mouse_entered(node_id : int, resource : FW_WorldmapNodeData, path : NodePath):
	node_mouse_entered.emit(path, node_id, resource)


func _on_node_mouse_exited(node_id : int, resource : FW_WorldmapNodeData, path : NodePath):
