    ...
    set(v):
        ...
        if v is FW_WorldmapViewItem:
            ...
            for i in siblings.size():
                ...
                if siblings[i] is FW_WorldmapViewItem:
...
func _handles(object : Object):
    return object is FW_WorldmapPath || object is FW_WorldmapGraph
...
func _forward_canvas_draw_over_viewport(overlay : Control):
    ...
    if edited_object is FW_WorldmapPath:
    ...
    if edited_object is FW_WorldmapGraph:
...
func _handle_non_marker_click(event : InputEventMouseButton) -> bool:
    ...
    if edited_object is FW_WorldmapViewItem:
...
func _arc_snap_end(pos : Vector2, snap_distance_squared : float) -> Vector2:
    ...
    if edited_object.mode != FW_WorldmapPath.PathMode.BEZIER:
        if dragging == 0:
    ...
    if edited_object.mode == FW_WorldmapPath.PathMode.LINE:
...
func _get_marker_positions() -> Array[Vector2]:
    ...
    if edited_object is FW_WorldmapPath:
        ...
        if edited_object.mode == FW_WorldmapPath.PathMode.ARC:
