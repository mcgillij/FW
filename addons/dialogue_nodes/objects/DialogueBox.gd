...
@export var data : FW_DialogueData :
	get:
		return data
	set(value):
		if _character_list_resource and _character_list_resource.is_connected("changed", _on_character_list_changed):
			_character_list_resource.disconnect("changed", _on_character_list_changed)

		data = value

		if data and not data.characters.is_empty():
			_character_list_resource = load(data.characters)
			if _character_list_resource and not _character_list_resource.is_connected("changed", _on_character_list_changed):
				_character_list_resource.connect("changed", _on_character_list_changed)
...
@export var custom_effects : Array[RichTextEffect] = [
	FW_RichTextWait.new(),
	FW_RichTextGhost.new(),
	FW_RichTextMatrix.new()
	]
...
var characters : Array[FW_Character]
## Displays the portrait image of the speaker in the [DialogueBox]. Access the speaker's texture by [member FW_DialogueBox.portrait.texture]. This value is automatically set while running a dialogue tree.
var portrait : TextureRect
## Displays the name of the speaker in the [DialogueBox]. Access the speaker name by [code]FW_DialogueBox.speaker_label.text[/code]. This value is automatically set while running a dialogue tree.
...
var _dialogue_parser : FW_DialogueParser
var _main_container : BoxContainer
var _sub_container : BoxContainer
var _wait_effect : FW_RichTextWait
var _character_list_resource: FW_CharacterList
...
func _enter_tree():
	...
	_dialogue_parser = FW_DialogueParser.new()
...
func _on_character_list_changed():
	#FW_Debug.debug_log(["Character list changed, updating FW_DialogueBox."])
	if _dialogue_parser and _character_list_resource:
		_dialogue_parser.characters = _character_list_resource.characters
		characters = _character_list_resource.characters
