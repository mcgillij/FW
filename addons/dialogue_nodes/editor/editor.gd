...
func _on_dialogue_background_input(event : InputEvent):
    if event is InputEventMouseButton:
        ...
        if _debug:
            FW_Debug.debug_log(['Dialogue canceled'])


func _on_dialogue_started(id : String):
    if _debug:
        FW_Debug.debug_log(['Dialogue started: ', id])


func _on_dialogue_option_selected(idx : int):
    if _debug:
        FW_Debug.debug_log(['Option selected. idx: ', idx, ', text: "', dialogue_box.options_container.get_child(idx).text, '"'])
...
func _on_dialogue_variable_changed(var_name : String, value):
    ...
    if _debug:
        FW_Debug.debug_log(['Variable changed:', var_name, ', value:', value])


func _on_dialogue_signal(value : String):
    if _debug:
        FW_Debug.debug_log(['Dialogue emitted signal with value:', value])
...
func _on_dialogue_ended():
    ...
    if _debug:
        FW_Debug.debug_log(['Dialogue ended'])
